...
public class Controller {
    ...
    ObservableList<Dictionary> dict = FXCollections.observableArrayList();

    private static Controller instance = new Controller();

    private static String filename = "vocabularay.txt";


    public void initialize() {

        File file = new File("vocabularay.txt");
        try {
            file.createNewFile();
        } catch (IOException e) {
            e.printStackTrace();
...
//                "Before he leaves, make sure his room is clean."));
    ...
    public void loadData() throws IOException {
        ...
        try {
            while ((input = br.readLine()) != null) {
                ...
                String wordFile = itemPieces[0];
                String partOfSpeechFile = itemPieces[1];
                String meaningFile = itemPieces[2];
                String exampleFile = itemPieces[3];
                 ...
                 */
                Vocab vocabFile = new Vocab(wordFile, partOfSpeechFile, meaningFile, exampleFile);

                observableList.add(vocabFile);

                    dictionary.add(wordFile, new Vocabulary(wordFile,
                            partOfSpeechFile,
                            meaningFile,
                            exampleFile));
    ...
    @FXML
    public void addVocab() throws IOException {
        if (!dictionary.edit(wordinput.getText()) &&
                ( !wordinput.getText().equals("") ||
                        !partOfSpeechinput.getText().equals("") ||
                        !meaninginput.getText().equals("") ||
                        !exampleinput.getText().equals(""))){
            ...
            try {
                ...
                Iterator<Vocab> iter = observableList.iterator();
                int i = 0;
                while (i < 1) {
                    //Vocab item = iter.next();
...
//                    exampleinput.getText()));
            ...
            try {
                ...
                System.err.println("Press Charactor");
            }
                show();
    ...
    public void storeData() throws IOException {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                bw.write(String.format("%s\t%s\t%s\t%s",
                        item.getWordinput(),
                        item.getPartOfSpeechinput(),
                        item.getMeaninginput(),
                        item.getExampleinput()));
                bw.newLine();
                i++;
            }
        }
    ...
    @FXML
    public  void removeVocab() throws IOException {
        if (dictionary.edit(wordinput.getText())){
            dictionary.remove(wordinput.getText());

            Path path = Paths.get(filename);
            FileWriter fileWriter2 = null;
            fileWriter2 = new FileWriter(filename);
            ...
            try {
                ...
                while(iter.hasNext()) {
                    Vocab item = iter.next();
                    if( !item.getWordinput().equals(wordinput.getText())){
                        bw.write(String.format("%s\t%s\t%s\t%s",
